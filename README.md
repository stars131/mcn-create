...
默认前端和 Route Handlers 使用 `src/server/services/mock-store.ts` 里的内存数据，便于 `npm run dev` 后立刻体验完整工作流。PostgreSQL/Redis 仍建议启动：它们用于验证 Prisma schema、migration、seed、pgvector 和未来 BullMQ worker 的真实运行环境。
...
- 页面与 API 默认读取内存 mock store：启动速度快，Playwright 和手动体验不依赖外部数据库状态。
- Prisma schema 与 seed 面向真实 PostgreSQL：用于验证企业级数据模型、pgvector 字段、RBAC、AgentRun、AuditLog、UsageEvent、CreditLedger 等持久化结构。
...
真实 PostgreSQL 验证使用 `.env.example` 中的默认连接：

```text
DATABASE_URL="postgresql://contentos:contentos@127.0.0.1:5432/contentos?schema=public"
REDIS_URL="redis://localhost:6379"
```

`prisma/seed.ts` 会写入一套可审计的真实库样例数据，包括组织、workspace、成员角色、品牌/人设、热点、选题、内容、风险检查、发布计划、指标、AgentRun、AgentOutput、AnalyticsReport、Recommendation、AuditLog 和 PromptTemplate。它不替代运行时 mock store；接入真实 repository 时，应以现有服务层接口为边界，把 `src/server/services/mock-store.ts` 逐步替换为 Prisma 查询。
