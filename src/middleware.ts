...
function getClientKey(request: NextRequest) {
  ...
  return createRateLimitKey({ clientId: ip, pathname: request.nextUrl.pathname });
}

function applySecurityHeaders(response: NextResponse, rateLimitDecision?: RateLimitDecision) {
  ...
  if (rateLimitDecision) {
    Object.entries(getRateLimitHeaders(rateLimitDecision)).forEach(([key, value]) => {
...
function authorizeApiRequest(request: NextRequest, userId: string, rateLimitDecision: RateLimitDecision) {
  ...
  if (!role) {
    return applySecurityHeaders(NextResponse.json({ error: "无权访问该 workspace" }, { status: 403 }), rateLimitDecision);
  }

  const permission = inferPermission(request);
  if (permission && !rolePermissions[role].includes(permission)) {
    return applySecurityHeaders(
      NextResponse.json({ error: `角色 ${role} 缺少权限 ${permission}` }, { status: 403 }),
      rateLimitDecision
    );
  }
...
function rateLimit(request: NextRequest) {
  ...
  const now = Date.now();
  clearExpiredRateLimitBuckets(buckets, now);
  return evaluateRateLimit({ buckets, key: getClientKey(request), now });
}
...
export function middleware(request: NextRequest) {
  ...
  if (pathname.startsWith("/api/")) {
    ...
    if (rateLimitDecision.limited) {
      return applySecurityHeaders(
        NextResponse.json({ error: "请求过于频繁，请稍后再试" }, { status: 429 }),
        rateLimitDecision
      );
    }

    if (MUTATING_METHODS.has(request.method) && !isSameOrigin(request)) {
      return applySecurityHeaders(NextResponse.json({ error: "跨站请求被拒绝" }, { status: 403 }), rateLimitDecision);
    }

    if (pathname.startsWith("/api/auth/")) {
      return applySecurityHeaders(NextResponse.next(), rateLimitDecision);
    }

    if (!userId) {
      return applySecurityHeaders(NextResponse.json({ error: "未登录" }, { status: 401 }), rateLimitDecision);
    }

    const denied = authorizeApiRequest(request, userId, rateLimitDecision);
    if (denied) {
      return denied;
    }
